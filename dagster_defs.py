...
def same_as_on_cron(
    _cron_schedule="* * * * *", _cron_timezone="America/New_York"
    ...
    return (
        ...
        & dg.AutomationCondition.all_deps_updated_since_cron(
            _cron_schedule, _cron_timezone
        )
    ).with_label(f"on_cron({_cron_schedule}, {_cron_timezone}")


def cron_no_since_at_all(
    _cron_schedule="* * * * *", _cron_timezone="America/New_York"
) -> dg.AutomationCondition:
    return (
        dg.AutomationCondition.in_latest_time_window()
...
@dg.asset(partitions_def=daily_partitions_def, automation_condition=same_as_eager)
def same_as_eager(every_minute):
    return


@dg.asset(
    partitions_def=daily_partitions_def, automation_condition=eager_no_initial_eval
)
...
@dg.asset(
    partitions_def=daily_partitions_def, automation_condition=eager_no_since_at_all
)
