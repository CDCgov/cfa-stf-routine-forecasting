...
def test_reads_file_uri_pointer_and_model_output(tmp_path):
    """Both pointer and Hubverse parquet can be addressed as file:// URIs."""
    hubverse_path = _write_hubverse(tmp_path / "hubverse.parquet", _hubverse_rows())
    pointer_path = _write_pointer(
        tmp_path / "latest.json",
        model_output_uri=hubverse_path.resolve().as_uri(),
    )
    source = HubversePointerNowcast(
        pointer_uri=pointer_path.resolve().as_uri(),
        forecast_spec=_spec(),
        source_label="NHSN Hubverse nowcast",
    )
...
def test_reads_az_uri_through_blobfuse_mount(tmp_path, monkeypatch):
    """az:// URIs are resolved through existing repo blobfuse mounts."""
    monkeypatch.chdir(tmp_path)
    mounted_root = tmp_path / "blobfuse" / "mounts" / "nhsn-prod"
    hubverse_path = mounted_root / "runs" / "weekly" / "hubverse.parquet"
    pointer_path = mounted_root / "latest" / "covid.json"
    hubverse_path.parent.mkdir(parents=True)
    pointer_path.parent.mkdir(parents=True)
    _write_hubverse(hubverse_path, _hubverse_rows())
    _write_pointer(
        pointer_path,
        model_output_uri="az://nhsn-prod/runs/weekly/hubverse.parquet",
    )
    source = HubversePointerNowcast(
        pointer_uri="az://nhsn-prod/latest/covid.json",
        forecast_spec=_spec(),
        source_label="NHSN Hubverse nowcast",
    )
...
def test_reads_az_uri_through_top_level_container_mount(tmp_path, monkeypatch):
    """az:// URIs are resolved through Azure Batch-style container mounts."""
    monkeypatch.chdir(tmp_path)
    mounted_root = tmp_path / "nhsn-prod"
    hubverse_path = mounted_root / "runs" / "weekly" / "hubverse.parquet"
    pointer_path = mounted_root / "latest" / "covid.json"
    hubverse_path.parent.mkdir(parents=True)
    pointer_path.parent.mkdir(parents=True)
