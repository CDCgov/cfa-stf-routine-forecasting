...
The run's `forecast_window` records that effective omission, while `batch_forecast_window` preserves the caller-requested configuration so all models remain in the same batch directory.
